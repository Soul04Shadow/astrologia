...
@router.get("/{profile_id}/varshphal")
def varshphal_for_profile(
    profile_id: int,
    year: int | None = None,
    ...
    try:
        ...
        v_data = compute_varshphal(chart, target_year=year)
    except Exception as e:
        raise HTTPException(status_code=422, detail=f"Varshphal calculation failed: {e}")
    return {"profile": {"id": profile.id, "name": profile.name}, "varshphal": v_data}
